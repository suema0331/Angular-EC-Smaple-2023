...
export class CartComponent {
  ...
  clickRemoveAllHandler($event: StoreProductExt): void {
    ...
    this.userCart = this.userCart.filter(s => s.store_product_id !== $event.store_product_id)
    this.cartPriceInfo = this.cartService.getCartPriceInfo();
  }
  ...
  openConfirmCartClearModal(): Observable<boolean> {
    this.checkCartClearModalRef = this.modalService.open(ConfirmCartClearModalComponent,
  ...
  hasCanNotOrderProducts(): boolean {
    if (this.storeMessage.store_available_flag === 0 || // business holiday
